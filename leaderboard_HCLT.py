...
def app():
    ...
    tasks = ["Ko-StrategyQA", "Markers_bm", "PublicHealthQA"]
    ...
    for subdir, dirs, files in os.walk(root_dir):
        if 'data/ONTHEIT' in os.path.relpath(subdir, root_dir):
            continue
        if "MIRACLRetrieval.json" in files:
            for file in files:
                for task in tasks:
                    if file == task + '.json':
                        with open(os.path.join(subdir, file)) as f:
                            ...
                            for top_k in top_k_types:
                                ...
                                f1_score = 2 * (results[score_types[top_k][1]] * results[score_types[top_k][0]]) / (results[score_types[top_k][1]] + results[score_types[top_k][0]]) if (results[score_types[top_k][1]] + results[score_types[top_k][0]]) > 0 else 0
                                data[task][top_k].append((os.path.relpath(subdir, root_dir), results[score_types[top_k][0]], results[score_types[top_k][1]], results[score_types[top_k][2]], f1_score))
    ...
    for task in tasks:
        ...
        for top_k in top_k_types:
            ...
            df = pd.DataFrame(data[task][top_k], columns=['Subdir', f'Recall_{top_k}', f'Precision_{top_k}', f'NDCG_{top_k}', f'F1_{top_k}'])
            ...
            for subdir, recall, precision, ndcg, f1 in data[task][top_k]:
                if subdir not in avg_data:
                    avg_data[subdir] = {k: [[], [], [], []] for k in top_k_types}
    ...
    for top_k in top_k_types:
        ...
        for model in avg_data:
            recall_avg = sum(avg_data[model][top_k][0]) / len(avg_data[model][top_k][0]) if avg_data[model][top_k][0] else 0
            precision_avg = sum(avg_data[model][top_k][1]) / len(avg_data[model][top_k][1]) if avg_data[model][top_k][1] else 0
            ndcg_avg = sum(avg_data[model][top_k][2]) / len(avg_data[model][top_k][2]) if avg_data[model][top_k][2] else 0
        ...
        avg_df = pd.DataFrame(avg_results, columns=['Model', f'Average Recall_{top_k}', f'Average Precision_{top_k}', f'Average NDCG_{top_k}', f'Average F1_{top_k}'])
